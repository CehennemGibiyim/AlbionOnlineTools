...
async function loadClassMarketData(itemId, itemName) {
    ...
    try {
        ...
        // Fetch history
        const histUrl = `${apiUrl}/history/${itemId}?locations=${cities}&time-scale=7`;
        const histRes = await fetch(histUrl);
        const histData = await histRes.json();

        // Fetch current prices
        const priceUrl = `${apiUrl}/prices/${itemId}?locations=Caerleon,Bridgewatch,FortSterling,Lymhurst,Martlock,Thetford,Brecilien`;
